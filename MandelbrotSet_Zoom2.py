    ...
    for i in range(1-isInit, fpz+1):
        ...
        for j in range(len(imgs)):
            if imgs[j][0] > r_zoom or (isInit and j == 0):
                ...
                if len(img) == 0:
                    img  = cv2.resize(imgs[j][1][ridx1:ridx2,cidx1:cidx2],
                                      (col,row), interpolation = cv2.INTER_AREA)
                else:
                    rimg = cv2.resize(imgs[j][1][ridx1:ridx2,cidx1:cidx2],
                                      (col,row), interpolation = cv2.INTER_AREA)
    ...
    for i in range(n_zoom + overlap - 1):
        if i < n_zoom:
            ...
            img, nmg = CalMandelbrotSet(row_zoom, col_zoom, x, y, r_zoom, N, R,
                                        colorlist)
            print("%3d, min n = %5d, max n = %5d" %
                  (i, np.min(nmg), np.max(nmg)))
        ...
        ZoomIn(i <= overlap, fpz, r, zoom, imgs, row, col,
               row_zoom, col_zoom, video)
        r *= zoom
